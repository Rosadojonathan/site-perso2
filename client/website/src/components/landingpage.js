          ...
          <div className="projects-grid">
            ...
            {this.state.postsHome.slice(-3).map(post=>
                ...
                <Card  key={post.id} shadow={5} style={{margin:'auto',marginBottom:'20px',minHeight:'280px'}} className="landing-cards">
                  <a href={`/blog/${post.path}`} style={{textDecoration:'none'}}>

                  <CardTitle style={{color:'black',fontSize:"1.4em",lineHeight: post.title.length>70 ? '0.9em':'1em'}}><b>{post.title}</b></CardTitle>
                  <CardText style={{fontSize:'1em'}}>{ post.description.length > maxCardTextLength ? post.description.substr(0,247) + '...' : post.description}</CardText>
                  </a>
                </Card>
